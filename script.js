...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    let headers = Object.keys(furnitureData[0]).filter(header => header !== 'data-prefix' && header !== 'Пользовательская оценка');
    ...
    furnitureData.forEach((item, rowIndex) => {
        ...
        headers.forEach(header => {
            ...
            if (header === 'Фото') {
                ...
                td.appendChild(checkbox);
                td.style.textAlign = 'center';
            } else if (header === 'Пользовательская оценка' && !isModernSofasPage) {
